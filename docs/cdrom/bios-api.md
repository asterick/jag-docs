...
In a **retail** Jaguar CD system the BIOS is installed automatically. In a
**development** system you must load it into DRAM manually using the provided
debugger script `CDBIOS??.DB` (the two digits are the version, e.g. `CDBIOS43.DB`
loads BIOS 4.3). See [Debugging](#debugging-with-the-cd-bios).
...
## Debugging with the CD BIOS

Two BIOS lines are distributed (2.x and 4.x). Development CD systems with the
**Butch 1** chip can use only revision 2; **Butch 2** systems (a modified
production-level case) support either. Format your data on a CD-R or the
[emulator](tools.md), then soft-load the BIOS with `load cdbios.xx.db`.

To capture a TOC for debugging, load the BIOS and run a small 68000 program:
...
```asm
        ...
        move.l    #$70C07,D_END      ; data organisation

        jsr       CD_setup
        move.w    #0,d0
        jsr       CD_mode            ; single speed, audio mode

        lea       $2C00,A0
        jsr       CD_getoc           ; read TOC to $2C00
...
Then `write toc.dat 2C00[400]` saves the TOC. Re-do this whenever disc data
changes. A debugging boot script looks like:
...
> **Never** call [`CD_getoc`](#cd_getoc) in shipping code — the CD Boot ROM loads
> the TOC of a booting disc to `$2C00` automatically. The bootcode must be < 64K
> and is responsible for loading further code/data segments.
...
| | |
|---|---|
| Input | `A0.L` start of destination buffer.<br>`A1.L` end of destination buffer (up to 64 bytes may be written **past** this address).<br>`D0.L` time code to start at: bit 31 set ⇒ "Just Seek"; remaining bits = `mm:ss:ff`.<br>`D1.L` partition-marker longword to search for (*`CD_initm` only*).<br>`D2.L` `N` where 2^N = circular buffer size in bytes; buffer aligned on a 2^(N+1) boundary; minimum `N` = 3; set 0 to disable (*`CD_initm` only*). |
...
| Input | none |
| Register usage | none |
| Returns | `A0.L` address of last written data.<br>`A1.L` approximate address of most recent error. |
...
| | |
|---|---|
| Input | `D0.W` 0 ⇒ return immediately, 1 ⇒ wait for completion.<br>`D1.W` session to spin up on. |
| Register usage | none |
| Returns | error code in `err_flag` |
